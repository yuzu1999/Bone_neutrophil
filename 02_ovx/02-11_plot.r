...
gene_len <- read.csv("OVX_len_names.csv",header=TRUE)


regulonActivity_byGroup <- read.csv("OVX_activity_average.csv",header=TRUE,row.names=1)

rownames(regulonActivity_byGroup) <- gene_len$new_name[match(rownames(regulonActivity_byGroup),gene_len$tfs)]



TFs_1 <- read.table("./OVX_GLM/Neu_VS_Non_neu_TFs.txt",header=TRUE)
TFs <- gene_len$new_name[match(TFs_1$TFs,gene_len$tfs)]

TFs_1 <- read.table("./OVX_GLM/Neu_Cluster_6_TFs.txt",header=TRUE)
TFs <- gene_len$new_name[match(TFs_1$TFs,gene_len$tfs)]

TFs_1 <- read.table("./OVX_GLM/Neu_Cluster_TFs.txt",header=TRUE)
TFs <- gene_len$new_name[match(TFs_1$TFs,gene_len$tfs)]
...
jpeg("02_OVX_AS_data_heatmap.jpg",height=700)
...
pdf("02_OVX_AS_data_heatmap.pdf",height=10.21)
pheatmap(result,scale = "row",
		cluster_cols=FALSE,cluster_rows=TRUE,
		show_colnames=FALSE,show_rownames=TRUE,
		annotation_col=col_ann,
...
jpeg("03_OVX_AS_data_heatmap.jpg",width=550)
pheatmap(result,scale = "row",
		cluster_cols=FALSE,cluster_rows=TRUE,
		show_colnames=FALSE,show_rownames=TRUE,
		annotation_col=col_ann,
...
pdf("03_OVX_AS_data_heatmap.pdf",width=8)
pheatmap(result,scale = "row",
		cluster_cols=FALSE,cluster_rows=TRUE,
		show_colnames=FALSE,show_rownames=TRUE,
		annotation_col=col_ann,
...
jpeg("04_OVX_AS_data_heatmap.jpg",width=550,height=800)
pheatmap(result,scale = "row",
		cluster_cols=FALSE,cluster_rows=TRUE,
		show_colnames=FALSE,show_rownames=TRUE,
		annotation_col=col_ann,
...
pdf("04_OVX_AS_data_heatmap.pdf",width=8,height=11.67)
pheatmap(result,scale = "row",
		cluster_cols=FALSE,cluster_rows=TRUE,
		show_colnames=FALSE,show_rownames=TRUE,
		annotation_col=col_ann,
...
grn <- read.table("/home/luchun/scRNA/Bone_neu/03_age_ovx2/pyscenic/data/OVX_adjacencies.tsv",sep='\t',header=T,stringsAsFactors=F)
head(grn)
...
for (tf in inregulons1) {
	...
	if (dim(tmp)[1] > ntop2) {
		...
		index <- which(tmp$target %in% c('Aph1b', 'Rfx3', 'Cdh13','Musk','Slc25a25'))
	...
	node2$node.colour[which(node2$tmp.target %in% c('Aph1b', 'Rfx3', 'Cdh13','Musk','Slc25a25'))] <- motif_target_col
...
pdf(paste("05_OVX_",inregulons1,"_regulon_netplot.pdf",sep=""),width=8)
p1
dev.off()
	
jpeg(paste("05_OVX_",inregulons1,"_regulon_netplot.jpg",sep=""),width=550)
p1
dev.off()
