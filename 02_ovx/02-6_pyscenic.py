...
#——————————————————————其余细胞随机抽样——————————————————————————————
adata_all = sc.read_h5ad("/home/luchun/scRNA/Bone_neu/02_ovx/04_sce_anno.h5ad")
adata_all
#AnnData object with n_obs × n_vars = 43755 × 2000


adata_neu=sc.read_h5ad("/home/luchun/scRNA/Bone_neu/02_ovx/05_neu_anno.h5ad")
adata_neu
...
adata_filtered
#AnnData object with n_obs × n_vars = 24431 × 2000
...
filtered_adata 
#View of AnnData object with n_obs × n_vars = 21281 × 2000
...
adata1
#AnnData object with n_obs × n_vars = 21281 × 19717
print(adata1.X)
...
adata
#AnnData object with n_obs × n_vars = 40605 × 19717
...
adata.write_h5ad('OVX_total_adata.h5ad',compression='gzip')
#adata = sc.read_h5ad("OVX_total_adata.h5ad")


import diopy
diopy.output.write_h5(adata,'OVX_total_adata.h5')
...
adata2
#View of AnnData object with n_obs × n_vars = 40605 × 16121
...
lp.create('/home/luchun/scRNA/Bone_neu/03_age_ovx2/pyscenic/data/OVX_total_adata.loom', adata.X.transpose(), row_attrs, col_attrs)
...
adata_counts
#AnnData object with n_obs × n_vars = 1200 × 19717
...
adata_counts=sampled_adata.raw.to_adata().copy()
adata_counts
#AnnData object with n_obs × n_vars = 2100 × 15981
print(adata_counts.X)
...
adata
#AnnData object with n_obs × n_vars = 3300 × 19717




adata.write_h5ad('OVX_sample_adata.h5ad',compression='gzip')

#adata = sc.read_h5ad("OVX_sample_adata.h5ad")
#adata

import diopy
diopy.output.write_h5(adata,'OVX_sample_adata.h5')
...
adata2 = adata[:, gene_saved2]
adata2
#View of AnnData object with n_obs × n_vars = 3300 × 16121
...
lp.create('/home/luchun/scRNA/Bone_neu/03_age_ovx2/pyscenic/data/OVX_sample_adata.loom', adata.X.transpose(), row_attrs, col_attrs)
