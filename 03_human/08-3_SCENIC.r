...
write.csv(len_names,"len_names.csv",quote=FALSE,row.names=FALSE)
...
regulonAUC <- get_regulons_AUC(sce_SCENIC, column.attr.name='aucell_regulonAUC')



pydata <- dior::read_h5('Human_total_adata.h5')
...
pydata$preAnno3[which(pydata$preAnno == "AZU1+KCNQ5+ progenitor")] <- "Neu_1"
pydata$preAnno3[which(pydata$preAnno == "PDE4D+MCU+ proliferating")] <- "Neu_2"
pydata$preAnno3[which(pydata$preAnno == "LTF+CAMP+ immature")] <- "Neu_3"
pydata$preAnno3[which(pydata$preAnno == "S100A12+MMP9+ immature")] <- "Neu_4"
pydata$preAnno3[which(pydata$preAnno == "S100A6+NCF1+ mature")] <- "Neu_5"
pydata$preAnno3[which(pydata$preAnno == "NAMPT+IFITM2+ mature")] <- "Neu_6"
pydata$preAnno3[which(pydata$preAnno == "NEAT1+FTH1+ mature")] <- "Neu_7"
...
write.csv(cellinfo,"Human_cellinfo.csv",quote=FALSE)
...
write.csv(AUC,"Human_activity.csv",quote=FALSE)
...
write.csv(regulonActivity_byGroup,"Human_activity_average.csv",quote=FALSE)
